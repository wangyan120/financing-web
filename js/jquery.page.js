	...
	var ms = {
		...
		fillHtml:function(obj,args){
			return (function(){
				...
				//上一页
				if(args.current > 1){
					obj.append('<a href="javascript:;" style="font-size: 12px" class="prevPage">上页</a>');
				}else{
					obj.remove('.prevPage');
					obj.append('<span class="disabled">上页</span>');
				}
				//中间页码
				...
				if(args.current < args.pageCount){
					obj.append('<a href="javascript:;" style="font-size: 12px" class="nextPage">下页</a>');
				}else{
					obj.remove('.nextPage');
					obj.append('<span class="disabled">下页</span>');
				}
				if(args.current < args.pageCount){
